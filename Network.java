...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    public boolean addUser(String name) {
        ...
        //add user to the next available space 
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for ( int i = 0; i < userCount; i ++) {
            userToCheck = users[i];
            if (!user.follows(userToCheck.getName()) && userToCheck != user) {

                mutualsToCheck = user.countMutual(userToCheck);
                if (mutualsToCheck > maxMutuals) {
                    maxMutuals = mutualsToCheck;
                    recommended = userToCheck;
                }
